...
impl ExecutorMetrics {
    ...
    pub fn execute_metered<'a, E, DB, O, Error, B>(
        &self,
        executor: E,
        input: BlockExecutionInput<'a, RecoveredBlock<B>>,
        state_hook: Box<dyn OnStateHook>,
    ) -> Result<BlockExecutionOutput<O>, Error>
    where
        E: Executor<
            DB,
            Input<'a> = BlockExecutionInput<'a, RecoveredBlock<B>>,
        ...
        B: reth_primitives_traits::Block,
    {
        ...
        let output =
            self.metered(input.block, || executor.execute_with_state_hook(input, wrapper))?;
...
mod tests {
    ...
    impl Executor<()> for MockExecutor {
        type Input<'a>
            = BlockExecutionInput<'a, RecoveredBlock>
        where
            Self: 'a;
